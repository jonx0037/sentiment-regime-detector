...
from sentiment_detector.services.sentiment_engine import SentimentEngine, SentimentScore as EngineSentimentScore
...
class SentimentService:
    ...
    async def get_current_sentiment(
        ...
        Computes real-time aggregates from sentiment_scores table.
        ...
        """
        asset_classes = [asset_class] if asset_class else ["equity", "crypto", "forex", "commodity"]
        results = []
        
        for ac in asset_classes:
            # Aggregate scores for this asset class
            query = (
                select(
            ...
            if stats.count == 0:
                # Skip asset classes with no data
                continue
            
            # Calculate momentum (simplified: just use avg compound for now)
            momentum = 0.0  # TODO: Calculate from time series in Phase 2
