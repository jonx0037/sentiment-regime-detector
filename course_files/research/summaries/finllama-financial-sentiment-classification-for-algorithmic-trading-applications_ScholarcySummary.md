...
There are multiple sources of financial news online that influence market movements and traders' decisions. This highlights the need for accurate sentiment analysis, along with appropriate algorithmic trading techniques, to make better-informed trading decisions. Standard lexicon-based sentiment approaches have demonstrated their power in aiding financial decisions. However, they are known to suffer from issues related to context sensitivity and word ordering. Large Language Models (LLMs) can also be used in this context, but they are not finance-specific and tend to require significant computational resources. To facilitate a finance-specific LLM framework, we introduce a novel approach based on the Llama 2 7B foundational model to leverage its generative nature and comprehensive language manipulation. This is achieved by fine-tuning the Llama2 7B model on a small portion of supervised financial sentiment analysis data to jointly handle the complexities of the financial lexicon and context, and by further equipping it with a neural network-based decision mechanism. Such a generator-classifier scheme, referred to as FinLlama, is trained not only to classify sentiment valence but also to quantify its strength, thus offering traders nuanced insight into financial news articles. Complementing this, parameter-efficient fine-tuning via LoRA optimises trainable parameters, thereby minimising computational and memory requirements without sacrificing accuracy. Simulation results demonstrate the proposed FinLlama's ability to provide a framework for enhanced portfolio management decisions and increased market returns. These results underscore FinLlama's ability to construct high-return portfolios that exhibit enhanced resilience, even during volatile periods and unpredictable market events.
...
The fine-tuning process uses the AdamW optimizer and the LoRA implementation to minimize trainable parameters, enabling it to run on a single A100 GPU.

### Results

The FinLlama model is evaluated against other sentiment analysis methods using finance-specific real-world metrics.
The results demonstrate FinLlama's ability to provide a framework for enhanced portfolio management decisions and increased market returns.
The model can construct high-return portfolios that exhibit greater resilience, even during volatile periods and unpredictable market events.

### Portfolio Construction

The long-short portfolio was constructed using sentiment to determine which companies should be in a long or short position.
The sentiment signal obtained from each of the five methods was used to construct five distinct portfolios.
Companies were ranked daily by sentiment, with those with the highest positive sentiment placed in long positions and those with the strongest negative sentiment placed in short positions.
An equally-weighted portfolio strategy was used, with 35% of companies in long positions and 35% in short positions.

### Sentiment Analysis

The sentiment analysis was performed using five methods: lexicon-based (HIV-4 and VADER) and deep learning (FinBERT and FinLlama).
The FinLlama model, a fine-tuned Llama-2, outperformed the other methods, achieving significantly higher cumulative returns and a higher Sharpe ratio while exhibiting lower volatility.
...
- To tackle these challenges, our work revisits the first principles of LLMs in order to align them to the task of financial sentiment analysis. This is achieved by using four labelled financial text datasets for fine-tuning the Llama 2 model. Such training on financial data equipped the model with the ability to understand the linguistic nuances present in the financial domain

### 34180 labelled samples

- This made it possible to alter the primary function of the model from text generation to sentiment classification. Our training data comprised four publicly available, labelled financial news datasets, yielding a comprehensive collection of 34,180 labelled samples. Each sample was annotated with one of the three labels: positive, negative, and neutral
