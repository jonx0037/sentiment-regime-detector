...
## Abstract
Previous research uses negative word counts to measure the tone of a text. We show that word lists developed for other disciplines misclassify common words in financial text. In a large sample of 10‐Ks during 1994 to 2008, almost three‐fourths of the words identified as negative by the widely used Harvard Dictionary are words typically not considered negative in financial contexts. We develop an alternative negative word list, along with five other word lists, that better reflect tone in financial text. We link the word lists to 10‐K filings, trading volume, return volatility, fraud, material weaknesses, and unexpected earnings.
...
These misclassifications can introduce noise into tone measurements and attenuate estimated regression coefficients.

### Development Of Alternative Word Lists
To address these limitations, an alternative negative word list, Fin-Neg, has been developed to better reflect tone in financial text.
The Fin-Neg list includes 2,337 words that typically carry negative financial implications.
...
These word lists have been linked to 10-K filings, trading volume, return volatility, fraud, material weakness, and unexpected earnings, and have been shown to produce significant relationships with these variables.

### Data Collection
The study requires at least 250 words in the MD&A section of 10-K filings, yielding 37,287 firm-year observations.
The parsing process excludes 10-K tables and exhibits, as they contain template language that is less meaningful for measuring tone.
...
### Methodology
The study uses a bag-of-words method, which requires parsing 10-K documents into word vectors and word counts.
Term weighting schemes acknowledge that raw word counts are not the best measure of a word's information content.
The study uses a modified term weighting scheme that adjusts for document length and examines both the simple proportion of words for a given tonal classification and tf. idf-weighted measures.
The study uses 1973 regressions with Newey–West 1987 standard errors and one lag.
The estimates are based on a sample of 50,115 10-Ks from 1994 to 2008.
...
A discipline-specific word list, such as the Fin-Neg list, can reduce measurement error and increase power by tailoring to business terminology.
...
The Fin-Neg list appears to capture useful information: firms with a lower percentage of pessimistic words have slightly negative returns, while those with a high percentage of negative words have sharply negative median returns.
...
The results show that the H4N-Inf list misclassifies many words as negative, potentially introducing noise into the tone measure.
...
The results show that some words, such as "mine" in the mining industry, can be misclassified as negative because of their common usage.
...
The weighted measures for both word lists are negative and significant, but the adjusted R2 remains low at around 2.6%.
...
### Trading Strategy
The study calculates the Fama-French four-factor portfolio returns by taking a long position in stocks with a low negative word count and a short position in stocks with a high negative word count.
Although the alphas across the four regressions are positive, none are statistically significant, indicating that the relationship between 1-year returns and negative word counts is insufficient to warrant active trading by investors.

### Results
The study finds that the word lists are highly significant in explaining post-event return volatility.
The coefficients on the word lists are positive, indicating that a higher proportion of positive, negative, or modal words is associated with greater stock return volatility in the year after the filing.
...
### Applications
The study uses word lists to predict quarterly earnings and finds that the coefficients on the Harvard and Fin-Neg word lists are positive and statistically significant, indicating that more negative words are associated with larger subsequent earnings surprises for the firm.
...
### Limitations
The Harvard dictionary's negative word list has limitations in a financial context, with almost three-fourths of its negative words not being negative in that context.
Common words like depreciation, liability, foreign, and board are not tonal when occurring in a 10-K filing.
The use of non-business word lists can lead to a high misclassification rate and spurious correlations.
...
Second, a term weighting scheme is developed to attenuate the impact of high-frequency words and allow less frequently used words to have a greater impact.
...
- Firms in the 10b-5 sample include Enron, Boston Chicken, and Cardinal Health. Our second sample considers [^Doyle_et+al_2007_a]) firms disclosing at least one material weakness in internal control between August 2002 and November 2005.10 These disclosures are an artifact of Sections 302 and 404 of the Sarbanes-Oxley Act.
...
## Contributions
- Summary Statistics for the1994 to 2008 10-K Sample. The first seven variables represent the proportion of occurrences for a given word list relative to the total number of words. The word lists are available in the Internet Appendix or at http://www.nd.edu/∼mcdonald/Word Lists.html. See the Appendix for definitions of the other variables. The sample sizes for the last three earnings-related variables are 28,679 for the full 10-K sample and 21,240 for the MD&A subsample.    Variable
