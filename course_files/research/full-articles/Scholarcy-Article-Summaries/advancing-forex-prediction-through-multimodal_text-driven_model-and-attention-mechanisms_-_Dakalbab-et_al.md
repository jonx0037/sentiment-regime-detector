...
## Quote
> This research proposes a multimodal Forex prediction model that combines qualitative sentiment data with quantitative technical analysis data, utilizing cross-modal attention mechanisms to enhance forecasting capabilities in the Forex market.
...
- Feature-specific scaling methods could be explored to ensure that technical indicators with different ranges, such as Relative Strength Index (RSI) and Moving Average Convergence Divergence (MACD), are normalized in a way that preserves their unique interpretations
...
This research work introduces a novel multimodal deep learning framework that integrates technical analysis and sentiment analysis through a cross-modal attention mechanism.
...
### Results
Experimental results highlight the superiority of the hybrid attention mechanism, which consistently outperformed single-modality models and other configurations across key metrics, such as Matthew’s correlation coefficient, accuracy, directional accuracy, and F1-score.
...
### Fusion Techniques
Multimodal learning models demonstrate enhanced performance compared to unimodal techniques, utilizing the depth of integrated data to attain more precise predictions and comprehensive representations.
Several fusion strategies are frequently utilized in multimodal models, including operation-based fusion, attention-based fusion, tensor-based fusion, subspace-based fusion, and graph-based fusion.
Operation-based fusion integrates feature vectors from many modalities using basic mathematical operations, while attention-based fusion prioritizes features across modalities, enabling the model to concentrate on the most important data for a certain task.
...
For instance, Farimani et al have made significant contributions to this field through several notable studies, investigating diverse methodologies for integrating sentiment and technical indicators to improve forecasting abilities.
...
The Windsor and Cao investigate multimodal learning through a fusion-based LSTM model that analyzes sentiment from social media platforms, including Twitter and Sina microblogs, when combined with technical indicators like EMA and MACD.
...
A multimodal model is developed, combining sentiment analysis and technical analysis using a cross-attention mechanism for fusion.
The model analyzes historical price data and news sentiment to predict Forex prices.
The cross-attention mechanism effectively comprehends the dynamic relationship between sentiment and technical indicators, improving the model's ability to accurately determine price movement direction.
...
The research compares the multimodal approach with singular module approaches and implements directional accuracy to evaluate the model's performance.
The results demonstrate the superiority of the multimodal method.
The research also examines the correlation between different labeling strategies and their impact on model performance, providing insights into the relationships between labeling approaches and facilitating the selection of complementary labeling strategies for robust model training.
...
The algorithm pairs each news story with the most recent price data before the article's publication to model short-term market reactions.
The sentiment analysis findings are matched with price movement data according to various scenarios, ensuring consistency between the datasets while preserving the nuances of the news sentiment data.
...
### Metrics
The study uses various metrics to evaluate the performance of the models, including Directional Accuracy (DA), Matthews Correlation Coefficient (MCC), accuracy, precision, recall, and F1-score.
DA evaluates the model's ability to predict directional price movements, while MCC provides a robust measure of classification accuracy, accounting for imbalanced datasets.

### Model Performance
The technical analysis model and sentiment analysis model are evaluated individually on three currency pairs (EUR/USD, GBP/USD, JPY/USD).
...
The multimodal approach, which combines technical indicators and sentiment analysis, is shown to outperform the individual models.
...
The model achieves an accuracy of 0.829 and an MCC of 0.744 for EUR/USD, an accuracy of 0.834 and an MCC of 0.752 for GBP/USD, and an accuracy of 0.852 and an MCC of 0.776 for JPY/USD.
...
The results show that the Self-Attention and Bi-Directional Cross-Attention modes outperform the No Attention mode, with the Bi-Directional Cross-Attention mode achieving the best performance.
The multimodal approach is shown to capture the complex interdependencies between market sentiment and historical trends, leading to improved predictive performance.
...
## Contributions
- In conclusion, the SA_BA Mode is superior to previous models, resulting in a level of predictive complexity that exceeds them. The advantages of a multi-attention framework for Forex market predictions are underscored by the comprehensive and sophisticated interaction of data streams that is facilitated by the integration of SA with bidirectional cross-attention. The significance of sophisticated attention mechanisms in improving the interpretability and accuracy of multimodal forecasting frameworks is underscored by the success of this model.
...
[^Barbieri_et+al_2020_a]: Barbieri, F., Camacho-Collados, J., Neves, L., &amp; Espinosa-Anke, L. (2020). TWEETEVAL: Unified benchmark and comparative evaluation for tweet classification. Findings of the association for computational linguistics findings of ACL: EMNLP 2020 (pp. 1644–1650). https://doi.org/10.18653/V1/2020.FINDINGS-EMNLP.148  [OA](https://doi.org/10.18653/V1/2020.FINDINGS-EMNLP.148)  [Scite](/scite_tallies?query=https://doi.org/10.18653/V1/2020.FINDINGS-EMNLP.148)
...
[^Dymova_et+al_2016_a]: Dymova, L., Sevastjanov, P., &amp; Kaczmarek, K. (2016). A FOREX trading expert system based on a new approach to the rule-base evidential reasoning. Expert Systems with Applications, 51, 1–13. https://doi.org/10.1016/j.eswa.2015.12.028  [OA](https://doi.org/10.1016/j.eswa.2015.12.028)  [Scite](/scite_tallies?query=https://doi.org/10.1016/j.eswa.2015.12.028)
...
[^Ma_et+al_2023_a]: Ma, N., et al. (2023). Comprehensive investigating of MMR gene in hepatocellular carcinoma with chronic hepatitis B virus infection in Han Chinese population. Frontiers in Oncology, 13. https://doi.org/10.3389/fonc.2023.1124459  [OA](https://doi.org/10.3389/fonc.2023.1124459)  [Scite](/scite_tallies?query=https://doi.org/10.3389/fonc.2023.1124459)
...
[^Yang_et+al_2018_a]: Yang, S. Y., Yu, Y., &amp; Almahdi, S. (2018). An investor sentiment reward-based trading system using Gaussian inverse reinforcement learning algorithm. Expert Systems with Applications, 114, 388–401. https://doi.org/10.1016/J.ESWA.2018.07.056  [OA](https://doi.org/10.1016/J.ESWA.2018.07.056)  [Scite](/scite_tallies?query=https://doi.org/10.1016/J.ESWA.2018.07.056)

[^Yang_2023_a]: Yang, H. (2023). Multimodal stock price forecasting using attention mechanism based on multi-task learning. In Proceedings of the Asia-Pacific web (APWeb) and web-age information management (WAIM) joint international conference on web and big data (pp. 454–468).  [OA](https://scholar.google.co.uk/scholar?q=Yang%2C%20H.%20Multimodal%20stock%20price%20forecasting%20using%20attention%20mechanism%20based%20on%20multi-task%20learning%202023) [GScholar](https://scholar.google.co.uk/scholar?q=Yang%2C%20H.%20Multimodal%20stock%20price%20forecasting%20using%20attention%20mechanism%20based%20on%20multi-task%20learning%202023) 
...
[^Zeng_2020_a]: Zeng, Z., &amp; Khushi, M. (2020). Wavelet denoising and attention-based RNN- ARIMA model to predict FOREX price. In Proceedings of the international joint conference on neural networks. Institute of Electrical and Electronics Engineers Inc.. https://doi.org/10.1109/IJCNN48605.2020.9206832  [OA](https://doi.org/10.1109/IJCNN48605.2020.9206832)  [Scite](/scite_tallies?query=https://doi.org/10.1109/IJCNN48605.2020.9206832)
